...
export async function publishVersionForUser(
  ...
  const eligibleFiles = safeFiles
    .filter((file) => file.path && !file.path.toLowerCase().endsWith('.md'))
    .filter((file) => isTextFile(file.path, file.contentType ?? undefined))
    .slice(0, MAX_FILES_FOR_EMBEDDING)

  const otherFiles = await Promise.all(
    eligibleFiles.map(async (file) => ({
      path: file.path,
      content: await fetchText(ctx, file.storageId),
    })),
  )
