...
export function verifyCheckpoint(checkpoint: MerkleCheckpoint): boolean {
  // Re-hash all events to detect tampering
  const eventsWithFreshHashes = checkpoint.events.map((e) => ({
    ...e,
    hash: hashEvent(e),
  }))
  const computed = computeMerkleRoot(eventsWithFreshHashes)
 ...
 */
export function saveCheckpoint(checkpoint: MerkleCheckpoint, filePath: string): void {
...
export async function main() {
  ...
  if (command === 'compute') {
    ...
    const checkpoint = JSON.parse(readFileSync(checkpointFile, 'utf-8')) as MerkleCheckpoint
